...
public class W3 implements Runnable {
    ...
    public void run() {
        try {
            for (int i = 0; i <= 2; i++) {
                System.out.println("<<<< ".concat(Thread.currentThread().getName()).concat(" Serving SAMOSA/CAKE/PASTRIES <<<<"));
                Thread.sleep(4000);
            }
        } catch (InterruptedException ie) {
            System.out.println("Interrupted Exception in W3 " + ie.getMessage());
        }
    }
}